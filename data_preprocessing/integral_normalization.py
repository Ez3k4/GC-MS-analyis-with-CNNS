...
from data_processing_module import load_csv_to_dataframe, bin_kovats_df, normalize_mz_values_int, save_df

""" This program normalizes all tsv files in one folder and saves them in specified folder.
...
for file_path in tqdm(file_paths, desc="Processing files", unit="file"):
    ...
    if answer.lower() == 'y':
        save_df(df, os.path.join(output_folder, f'{name}_int{ext}'))
        print(f'{name} was standarized, saved as {name}_int{ext} at {output_folder}')
    else:
        print("exit")
        break
